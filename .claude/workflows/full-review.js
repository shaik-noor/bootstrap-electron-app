export const meta = {
  name: 'full-review',
  description: 'Pre-release quality gate: parallel code review, security audit, accessibility check, performance review, and test coverage check across all changed files',
...
log(`Reviewing ${changedFiles.length} changed file(s). Test status: ${scope?.testStatus ?? 'unknown'}.`)
  ...
  properties: {
    ...
    blockers: {
      ...
      items: {
        ...
        properties: { file: { type: 'string' }, line: { type: 'number' }, description: { type: 'string' } },
      },
    },
    warnings: {
      type: 'array',
...
const allBlockers = reviews.filter(Boolean).flatMap((r) => (r.blockers ?? []).map((b) => ({ ...b, dimension: r.dimension })))
const allWarnings = reviews.filter(Boolean).flatMap((r) => (r.warnings ?? []).map((w) => ({ ...w, dimension: r.dimension })))
