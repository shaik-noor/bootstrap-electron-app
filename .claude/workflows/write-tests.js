export const meta = {
  name: 'write-tests',
  description:
    'Write tests for a specific file or feature. Pass the target file path or feature description as args.',
  whenToUse:
    'Use after implementing a new feature, IPC channel, component, or DatabaseService method to write the corresponding tests',
  phases: [
...
if (!analysis?.vitestConfigExists) {
  log(
    'ERROR: vitest.config.ts not found. Run /setup-tests first to scaffold the test infrastructure.'
  )
...
await pipeline(batches, (batch, _, batchIndex) =>
  agent(
